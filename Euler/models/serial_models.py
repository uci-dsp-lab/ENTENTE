...
class GraphGRU(nn.Module):
    def __init__(self, in_size, hidden_size, n_layers=1, asl=True, device=None):
        ...
        for i in range(self.n_layers):
            if i==0:
                self.weight_xz.append(GCNConv(in_size, hidden_size, add_self_loops=asl).to(self.device))
                self.weight_hz.append(GCNConv(hidden_size, hidden_size, add_self_loops=asl).to(self.device))
                self.weight_xr.append(GCNConv(in_size, hidden_size, add_self_loops=asl).to(self.device))
                self.weight_hr.append(GCNConv(hidden_size, hidden_size, add_self_loops=asl).to(self.device))
                self.weight_xh.append(GCNConv(in_size, hidden_size, add_self_loops=asl).to(self.device))
                self.weight_hh.append(GCNConv(hidden_size, hidden_size, add_self_loops=asl).to(self.device))
            else:
                self.weight_xz.append(GCNConv(hidden_size, hidden_size, add_self_loops=asl).to(self.device))
                self.weight_hz.append(GCNConv(hidden_size, hidden_size, add_self_loops=asl).to(self.device))
                self.weight_xr.append(GCNConv(hidden_size, hidden_size, add_self_loops=asl).to(self.device))
                self.weight_hr.append(GCNConv(hidden_size, hidden_size, add_self_loops=asl).to(self.device))
                self.weight_xh.append(GCNConv(hidden_size, hidden_size, add_self_loops=asl).to(self.device))
                self.weight_hh.append(GCNConv(hidden_size, hidden_size, add_self_loops=asl).to(self.device))
    ...
    def forward_once(self, x, ei, h):
        ...
        for i in range(self.n_layers):
            if i == 0:
                # print(x.is_cuda, ei.is_cuda, h.is_cuda)
                # print(.is_cuda)
                # print(next(self.weight_xz[i].parameters()).device)
...
'''
class VGRNN(Euler_Embed_Unit):
    def __init__(self, data_load, data_kws, h_dim, z_dim, grnn=True, pred=True, device=None, h0=None):
        super(VGRNN, self).__init__()
        self.device = device
        self.h_dim = h_dim
        self.grnn = grnn
        self.data = data_load(**data_kws).to(self.device)
        self.h = None if h0 is None else h0.to(self.device)
        ...
        self.recurrent = GraphGRU(
            h_dim*2, h_dim, device=self.device
        )
    ...
    def forward_once(self, x, ei, h):
        if type(h) == type(None):
            h = torch.zeros((x.size(0), self.h_dim)).to(self.device)

        x = self.phi_x(x)
        gcn_x = torch.cat([x,h], dim=1)#.to(self.device)
...
class SparseEGCN_H(EGCN_H):
    def __init__(self, x_dim, h_dim, z_dim, pred=False, device=None):
        # Why do they insist on doing it this way. Fixing it
        args = SN(
            feats_per_node=x_dim,
            layer_1_feats=h_dim,
