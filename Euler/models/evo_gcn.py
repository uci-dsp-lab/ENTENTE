...
class LP_EGCN_o(EGCN_o):
    def __init__(self, data_load, data_kws, h_dim, z_dim, inner_prod=False, device=None):
        self.device = device
        self.h_dim = h_dim
    ...
    def convert_to_dense(self, ei, data):
        ei = add_remaining_self_loops(ei, num_nodes=self.data.num_nodes)[0]
        a = to_dense_adj(ei, max_num_nodes=self.data.num_nodes)[0]
        d = a.sum(dim=1)
        d = 1/torch.sqrt(d)
        d = torch.diag(d)
        ahat = d @ a @ d
        return [ahat], [torch.eye(self.data.num_nodes)]
    ...
    def inner_forward(self, mask_enum):
        ...
        for i in range(self.data.T):
            ...
            tr_adjs, tr_xs = self.convert_to_dense(ei, xs)
            z = self.forward_once(tr_adjs, tr_xs)
            zs.append(z)
        return torch.stack(zs)

    def forward_once(self, A_list, Nodes_list):
        '''
