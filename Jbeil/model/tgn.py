...
class TGN(torch.nn.Module):
  def __init__(self, neighbor_finder, node_features, edge_features, device, n_layers=2,
    ...
    self.time_encoder = TimeEncode(dimension=self.n_node_features, device=device)
    self.memory = None
    ...
    if self.use_memory:
      ...
      self.message_function = get_message_function(module_type=message_function, raw_message_dimension=raw_message_dimension,
                                                   message_dimension=message_dimension)
  ...
  def save_states(self):
    return self, deepcopy(self.state_dict())
  def load_states(self, state_dict):
    # print(state_dict2.shape)
